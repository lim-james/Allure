...
void EventsManager::EventCallback::operator()(Events::Event* event) const {
...
void EventsManager::Trigger(std::string const& name, Events::Event* const event) {
	for (auto& callback : emptyCallbacks[name])
		(*callback)();
...
void EventsManager::Queue(std::string const& name, Events::Event* const event) {
