...
void TilemapEditorManager::SetPreview(SpriteRender * const render) {
	previewRender = render;
	previewTransform = entities->GetComponent<Transform>(render->entity);
}

void TilemapEditorManager::PreviewMouseOver() {
	editorCamera->enabled = false;
}

void TilemapEditorManager::PreviewMouseOut() {
	editorCamera->enabled = true;
}

void TilemapEditorManager::PreviewClick() {
...
void TilemapEditorManager::Awake() {
	EventsManager::Get()->Subscribe("DROP_INPUT", &TilemapEditorManager::DropEvent, this);
	EventsManager::Get()->Subscribe("KEY_INPUT", &TilemapEditorManager::KeyEvent, this);
	EventsManager::Get()->Subscribe("CURSOR_POSITION_INPUT", &TilemapEditorManager::CursorPositionHandler, this);
	EventsManager::Get()->Subscribe("MOUSE_BUTTON_INPUT", &TilemapEditorManager::MouseButtonHandler, this);
}

void TilemapEditorManager::Start() {
	layerHeight = index = size = 0;
	layerLabel->text = "LAYER 0";
}

void TilemapEditorManager::DropEvent(Events::Event * event) {
...
void TilemapEditorManager::KeyEvent(Events::Event * event) {
...
void TilemapEditorManager::CursorPositionHandler(Events::Event * event) {
	Events::CursorPositionInput* const input = static_cast<Events::CursorPositionInput*>(event);
	cursorPosition = input->position;
}

void TilemapEditorManager::MouseButtonHandler(Events::Event * event) {
...
void TilemapEditorManager::UpdateIndex() {
...
void TilemapEditorManager::CreateLayer(int const & z) {
